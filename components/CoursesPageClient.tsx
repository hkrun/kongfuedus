...
type CoursesPageClientProps = {
  initialCourses: any[];
  initialCategoryInfo: Record<string, any>;
...
export default function CoursesPageClient({
  ...
  const [currentLocale, setCurrentLocale] = useState<SupportedLocale>(initialLocaleKey);
  const [isContactModalOpen, setIsContactModalOpen] = useState(false);
  const [isFAQModalOpen, setIsFAQModalOpen] = useState(false);
  const [categoryInfo, setCategoryInfo] = useState<Record<string, any>>(initialCategoryInfo);
  ...
  const renderStars = (rating: number) => {
    ...
    for (let i = 0; i < fullStars; i++) stars.push(<i key={i} className="fa fa-star"></i>);
    if (hasHalfStar) stars.push(<i key="half" className="fa fa-star-half-o"></i>);
    const emptyStars = 5 - Math.ceil(rating);
    for (let i = 0; i < emptyStars; i++) stars.push(<i key={`empty-${i}`} className="fa fa-star-o"></i>);
    return stars;
  };
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mb-10 overflow-x-auto scrollbar-hide">
            <div className="flex space-x-2 md:space-x-4 min-w-max justify-center pb-2">
              <button className={`category-filter px-4 py-2 rounded-full font-medium transition-all duration-300 ${activeCategory === 'all' ? 'text-white' : 'text-gray-700 hover:bg-gray-300'}`} style={{ backgroundColor: activeCategory === 'all' ? 'rgb(26 54 93 / var(--tw-bg-opacity, 1))' : 'rgb(229 231 235 / var(--tw-bg-opacity, 1))' }} onClick={() => setActiveCategory('all')}>{t('courses.allCourses')}</button>
              <button className={`category-filter px-4 py-2 rounded-full font-medium transition-all duration-300 ${activeCategory === 'striking' ? 'text-white' : 'text-gray-700 hover:bg-gray-300'}`} style={{ backgroundColor: activeCategory === 'striking' ? 'rgb(26 54 93 / var(--tw-bg-opacity, 1))' : 'rgb(229 231 235 / var(--tw-bg-opacity, 1))' }} onClick={() => setActiveCategory('striking')}><i className="fa fa-fist-raised mr-2"></i>{t('courses.categories.striking')}</button>
              <button className={`category-filter px-4 py-2 rounded-full font-medium transition-all duration-300 ${activeCategory === 'taiji' ? 'text-white' : 'text-gray-700 hover:bg-gray-300'}`} style={{ backgroundColor: activeCategory === 'taiji' ? 'rgb(26 54 93 / var(--tw-bg-opacity, 1))' : 'rgb(229 231 235 / var(--tw-bg-opacity, 1))' }} onClick={() => setActiveCategory('taiji')}><i className="fa fa-circle-o-notch mr-2"></i>{t('courses.categories.taiji')}</button>
              <button className={`category-filter px-4 py-2 rounded-full font-medium transition-all duration-300 ${activeCategory === 'weapons' ? 'text-white' : 'text-gray-700 hover:bg-gray-300'}`} style={{ backgroundColor: activeCategory === 'weapons' ? 'rgb(26 54 93 / var(--tw-bg-opacity, 1))' : 'rgb(229 231 235 / var(--tw-bg-opacity, 1))' }} onClick={() => setActiveCategory('weapons')}><i className="fa fa-balance-scale mr-2"></i>{t('courses.categories.weapons')}</button>
              <button className={`category-filter px-4 py-2 rounded-full font-medium transition-all duration-300 ${activeCategory === 'health' ? 'text-white' : 'text-gray-700 hover:bg-gray-300'}`} style={{ backgroundColor: activeCategory === 'health' ? 'rgb(26 54 93 / var(--tw-bg-opacity, 1))' : 'rgb(229 231 235 / var(--tw-bg-opacity, 1))' }} onClick={() => setActiveCategory('health')}><i className="fa fa-heartbeat mr-2"></i>{t('courses.categories.health')}</button>
              <button className={`category-filter px-4 py-2 rounded-full font-medium transition-all duration-300 ${activeCategory === 'combat' ? 'text-white' : 'text-gray-700 hover:bg-gray-300'}`} style={{ backgroundColor: activeCategory === 'combat' ? 'rgb(26 54 93 / var(--tw-bg-opacity, 1))' : 'rgb(229 231 235 / var(--tw-bg-opacity, 1))' }} onClick={() => setActiveCategory('combat')}><i className="fa fa-shield mr-2"></i>{t('courses.categories.combat')}</button>
              <button className={`category-filter px-4 py-2 rounded-full font-medium transition-all duration-300 ${activeCategory === 'mixed' ? 'text-white' : 'text-gray-700 hover:bg-gray-300'}`} style={{ backgroundColor: activeCategory === 'mixed' ? 'rgb(26 54 93 / var(--tw-bg-opacity, 1))' : 'rgb(229 231 235 / var(--tw-bg-opacity, 1))' }} onClick={() => setActiveCategory('mixed')}><i className="fa fa-random mr-2"></i>{t('courses.categories.mixed')}</button>
            </div>
          </div>
          <div className="mb-8">
            <div className="flex flex-wrap items-center justify-between">
              <div>
                <h2 className="font-bold text-2xl mb-2" style={{ color: 'rgb(26 54 93 / var(--tw-text-opacity, 1))', fontFamily: 'Montserrat, sans-serif' }}>{categoryInfo[activeCategory]?.title || activeCategory}</h2>
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-12">
            ...
            <div>
              ...
              <ul className="space-y-3">
                {(['striking', 'taiji', 'weapons', 'health', 'combat', 'mixed'] as const).map((c) => (
                  <li key={c}><Link href="#" className="text-gray-300 hover:text-orange-500 transition-all duration-300">{t(categoryKeys[c])}</Link></li>
                ))}
              </ul>
            </div>
            <div>
